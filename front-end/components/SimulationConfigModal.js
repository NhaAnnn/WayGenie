...
const SimulationConfigModal = ({ isVisible, onClose, onSimulationApplied }) => {
  ...
  const handleUpdateSimulation = async (item) => {
    ...
    if (isAqi) {
      ...
      const parsedIncident = editingTrafficIncident; // Giả sử incident là chuỗi hoặc số
        ...
        { text: "Hủy", style: "cancel" },
        {
          ...
          onPress: async () => {
            ...
            try {
              ...
              Alert.alert(
                "Thành công",
                `Đã xóa mô phỏng ${isAqi ? "AQI" : "tuyến đường"}.`
              );
              if (isAqi) {
                setActiveAqiSimulations((prev) =>
    ...
    >
      ...
      >
        <View style={styles.modalView}>
          <Text style={styles.modalTitle}>Quản lý Mô Phỏng</Text>
