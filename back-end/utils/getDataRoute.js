...
async function processData(nodeFile, linkFile, shapeFilePath) {
  ...
  try {
    ...
    console.log("NODE_FILE:", nodeFile);
    console.log("LINK_FILE:", linkFile);
    console.log("SHAPEFILE_PATH:", shapeFilePath || "Không có shapefile");

    // Kiểm tra file CSV bắt buộc
    for (const file of [nodeFile, linkFile]) {
      if (!file) {
        throw new Error(`Đường dẫn file không được định nghĩa: ${file}`);
      }
    ...
    // Khởi tạo GDAL CRS nếu có shapefile
    if (shapeFilePath && fs.existsSync(shapeFilePath)) {
      try {
        gdalSourceCRS = gdal.SpatialReference.fromProj4(
          SOURCE_CRS_PROJ4_STRING
        );
        gdalTargetCRS = gdal.SpatialReference.fromEPSG(4326);
        ...
        gdalCoordTransform = null;
      }
    } else {
      console.warn(
        "Không có SHAPEFILE_PATH hoặc file shapefile không tồn tại. Bỏ qua xử lý geometry."
      );
    }

    // Đọc và xử lý node.csv
    console.log(`Đọc file NODE từ: ${nodeFile}`);
    const nodes = [];
    ...
    // Đọc và xử lý link.csv
    console.log(`Đọc file LINK từ: ${linkFile}`);
    const linksData = [];
    await new Promise((resolve, reject) => {
      fs.createReadStream(linkFile)
        .pipe(
          csv({
            separator: ",",
            skipLines: 1,
    ...
    if (shapeFilePath && fs.existsSync(shapeFilePath) && gdalCoordTransform) {
      try {
        console.log(`Đọc và xử lý Geometry từ Shapefile: ${shapeFilePath}`);
        dataset = gdal.open(shapeFilePath);
        console.log("Shapefile đã được mở thành công!");
        const layer = dataset.layers.get(0);
        console.log(
          `Tên lớp: ${layer.name}, Số lượng features: ${layer.features.count()}`
        );

        if (layer.srs) {
          console.log(`GDAL tự động nhận diện CRS: ${layer.srs.toProj4()}`);
          console.log(`Lưu ý: Buộc chuyển đổi từ ESRI:53004.`);
        } else {
          console.warn(
            "Không tìm thấy CRS trong Shapefile. Buộc chuyển đổi từ ESRI:53004."
          );
        }
        ...
        layer.features.forEach((feature, index) => {
          ...
          if (isNaN(linkIdInShp)) {
            console.warn(
              `Shapefile Parsing Warning: 'NO' field "${rawLinkIdInShp}" for feature ${index} is not a valid number. Skipping.`
            );
            return;
          }
          ...
          if (index < 10 || linkIdInShp === 5348) {
            ...
            if (geometry) {
              ...
              try {
                ...
                console.warn(
                  `   Error converting initial geometry to GeoJSON: ${e.message}`
                );
              }
            } else {
              console.warn(
          ...
          if (link && geometry && !geometry.isEmpty()) {
            try {
              geometry.transform(gdalCoordTransform);
              const geoJsonObj = geometry.toObject();
              ...
              if (geoJsonObj.type === "LineString" && geoJsonObj.coordinates) {
                ...
                if (uniqueCoordinates.length < 2) {
                  console.warn(
                    `Link ID ${linkIdInShp}: LineString chỉ có 1 đỉnh duy nhất sau khi lọc trùng lặp. Bỏ qua geometry.`
                  );
                  link.geometry = null;
                } else {
                  link.geometry = {
                ...
                console.warn(
                  `Link ID ${linkIdInShp}: Loại geometry không hỗ trợ: ${geoJsonObj.type}. Bỏ qua geometry.`
                );
                link.geometry = null;
              }
                  ...
                  console.log(
                    `   Transformed GeoJSON Type: ${
                      ...
                      link.geometry.coordinates
                        ? JSON.stringify(link.geometry.coordinates.slice(0, 5))
                        : "No coordinates array"
                    }`
                  );
                } else {
              ...
              console.error(
                `Lỗi chuyển đổi geometry cho link ID ${linkIdInShp}: ${transformError.message}. Bỏ qua geometry.`
              );
              link.geometry = null;
            }
          } else {
